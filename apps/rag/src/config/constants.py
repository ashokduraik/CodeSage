"""Default tuning values for CodeSage RAG.
...
in ``apps/rag/.env.example`` because they do not change from one deployment to another.

Environment-specific values (connections, secrets, endpoints, model ids, ports) and per-deploy
feature toggles live in ``.env.example`` instead. See ``.cursor/rules/rag-config.mdc`` for the
