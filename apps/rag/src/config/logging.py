"""Structured, beginner-friendly logging for the RAG service."""
...
RAG_LOG_FORMAT = "%(asctime)s  %(levelname)-5s  [RAG]  %(message)s"
...
def get_indexing_logger() -> logging.Logger:
    """Return the shared RAG application logger.

    @returns Logger whose output uses the unified ``[RAG]`` format.
    """
...
def _build_console_handler() -> logging.Handler:
    """Create a stderr stream handler with the unified RAG log format.

    @returns Configured stream handler.
    """
    formatter = logging.Formatter(fmt=RAG_LOG_FORMAT, datefmt=RAG_LOG_DATEFMT)
...
def configure_logging(settings: Settings) -> None:
    """Configure uniform console logging for the entire RAG process.

    Applies the same ``TIMESTAMP  LEVEL  [RAG]  message`` format to application,
