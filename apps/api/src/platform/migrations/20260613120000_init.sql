...
-- pgvector is only required for Phase 1 vector search; skip gracefully if not installed.
DO $$ BEGIN
  CREATE EXTENSION IF NOT EXISTS vector;
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'pgvector not installed — vector search will be unavailable (install it before Phase 1). Error: %', SQLERRM;
END $$;
