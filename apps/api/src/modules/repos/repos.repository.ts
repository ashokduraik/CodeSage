 ...
 */
export async function insertRepo(db: Sql, params: InsertRepoParams): Promise<RepoRow> {
  const rows = await db<RepoRow[]>`
    INSERT INTO repos (
 ...
 * @param repoId - Repo UUID.
 */
export async function setRepoConnecting(db: Sql, repoId: string): Promise<void> {
  await db`
    UPDATE repos
