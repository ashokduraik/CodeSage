...
_DOCKER_HOSTS = frozenset({"tei", "postgres", "db", "redis", "engine", "api", "web"})
...
def explain_failure(
    ...
    *,
    ...
    if "token_enc_key must decode" in lowered:
        return (
            "Encryption key mismatch — TOKEN_ENC_KEY in apps/engine/.env "
            "must match apps/api"
        )
...
def _explain_connect_failure(raw: str, settings: Settings | None) -> str:
    ...
    if settings is not None and settings.tei_base_url:
        ...
        # Hostnames like ``tei`` or ``engine`` resolve inside Docker Compose but not when
