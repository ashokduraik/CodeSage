...
> `investigation_trace` via `20260717120000_qa_playbooks_investigation_trace.sql`)  
...
response (`needs_review`) instead of inventing detail. Assistant rows from the agent QA path store
an `investigation_trace` JSONB (tool loop; contract `InvestigationTrace`). A `stopped` assistant
...
| `investigation_trace` | `jsonb` | YES | — | `InvestigationTrace` (agent tool loop; schema in `contracts/openapi.engine.yaml`). Populated on grounded agent answers via SSE `metrics.investigationTrace` |
