...
- **FR-20** Audience dispatch sends **developer/code** questions to an agent-orchestrated evidence
  loop (bounded symbol, keyword, vector, hybrid, graph, and exact-read tools; deterministic
  confidence; max-iteration abstain — ADR 0026) and **product/usage** questions to structured
  knowledge tools when Phase 6 is implemented.
