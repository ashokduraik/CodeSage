...
Canonical guide for **beginner-friendly, leak-free** logging in `apps/engine`. AI agents
...
```
TIMESTAMP  LEVEL  [ENGINE]  message
```

Example:

```
2026-07-04 16:59:43  INFO   [ENGINE]  Connected to PostgreSQL at localhost:5432/codesage_db
2026-07-04 16:59:43  INFO   [ENGINE]  Application startup complete.
```

- Configured in `config.logging.configure_logging()` — do not use uvicorn default logging.
- Start the service via `python -m api.run` (see `package.json` `dev:engine`) with
  `log_config=None`.
- Never mix formats like `INFO:     Uvicorn running…` alongside `[ENGINE]` lines.
...
- Module: [`apps/engine/src/config/logging.py`](../apps/engine/src/config/logging.py)
...
2. `log_failure()` — one ERROR summary line **then** sanitized traceback lines (each prefixed `[ENGINE]`)
...
User-facing indexing timeline lives in `repo_indexing_events` (append-only, full history). Console `[ENGINE]` logs are ephemeral; DB events mirror the same step messages for UI later.
...
5. Do not add loggers that bypass `configure_logging()` / the `[ENGINE]` format.
