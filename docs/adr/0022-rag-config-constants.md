...
- **Related:** ADR 0020 (hybrid retrieval), ADR 0021 (retrieval quality pass), ADR 0015 (single
  Python deployable); `.cursor/rules/engine-config.mdc`, `.cursor/rules/env-example-sync.mdc`

## Context

`apps/engine/.env.example` had grown to ~45 variables, most of them retrieval and worker **tuning
...
(`apps/engine/src/config/__init__.py`). These values are standard across deployments — an operator
...
A new rule `.cursor/rules/engine-config.mdc` governs which bucket a new knob belongs in, and
